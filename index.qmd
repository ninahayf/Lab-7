...
Denver_and_New_Orleans_Elevation <- ggplot() +
...
ggsave("images/Denver_and_New_Orleans_Elevation.png", 
       plot = Denver_and_New_Orleans_Elevation)
```
...
TAVG_over_T <- ggplot(temp_data_clean, aes(x = Year, y = Average_Temperature, color = City)) +
...
# Plot for Denver
Denver_Temp_Data <- ggplot(denver_data, aes(x = Year, y = Value, color = City)) +
...
ggsave("images/Denver_Temp_Data.png",
       plot = Denver_Temp_Data)
ggsave("images/New_Orleans_Temp_Data.png", 
       plot = New_Orleans_Temp_Data)
```
...
Humidity <- ggplot(humid_long, aes(x = Month, y = Humidity, color = CityTime, group = CityTime)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
...
elev_tavg <- ggplot(final_data_clean, aes(x = Elevation_ft, y = TAVG, color = City)) +
  geom_point() +
...
ggsave("images/elev_tavg.png", 
       plot = elev_tavg)
```

```{r}
# Scatter plot for UHI Effect vs TAVG
UHI_TAVG <- ggplot(final_data_clean, aes(x = UHI_Effect, y = TAVG, color = City)) +
  geom_point() +
  geom_smooth(method = "lm", col = "purple") +
  labs(title = "UHI Effect vs. Temperature (TAVG)", x = "UHI Effect (°F)", y = "Temperature (TAVG)") +
  scale_color_manual(values = c("NEW ORLEANS" = "deeppink", "DENVER" = "skyblue"))

ggsave("images/UHI_TAVG.png", 
       plot = UHI_TAVG)
```
